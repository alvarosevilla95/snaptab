...
local function close_unlisted_windows()
  local windows = vim.api.nvim_list_wins()
  for _, win in ipairs(windows) do
    local buf = vim.api.nvim_win_get_buf(win)
    if not vim.api.nvim_get_option_value("buflisted", { buf = buf }) then vim.api.nvim_win_close(win, false) end
  end
end
...
local function restore_leaf(layout)
  ...
  if layout.current then return vim.api.nvim_get_current_win() end
end
...
local function restore_node(layout)
  ...
  for i, _ in ipairs(layout.children) do
    if i > 1 then vim.cmd(layout.type == "col" and "belowright split" or "belowright vsplit") end
  ...
  for i, child in ipairs(layout.children) do
    ...
    if _winid then winid = _winid end
  end
